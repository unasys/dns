...
class DisciplinesTable extends React.Component {
  ...
  requestData(pageSize, page) {
    return fetchDisciplines(this.props.projectId, this.source.token, pageSize * page, pageSize)
      .then(payload => {
        this.setState({
  ...
  render() {
    return (
      <div className="margin-top-left" >
        ...
        <h4 className="subtitle is-4">{'Total Disciplines: ' + this.state.total}</h4>
      </div>
    );
  }
}

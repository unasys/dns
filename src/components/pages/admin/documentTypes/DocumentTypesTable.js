...
class DocumentTypesTable extends React.Component {
  ...
  requestData(pageSize, page) {
    return fetchDocumentTypes(this.props.projectId, this.source.token, pageSize * page, pageSize)
      .then(payload => {
        this.setState({
  ...
  render() {
    return (
      <div className="margin-top-left">
        ...
        <h4 className="subtitle is-4">{'Total Document Types: ' + this.state.total}</h4>
      </div>
    );
  }
}
